...
export function IncomingInvoiceForm({
    ...
    const { data, setData, post, patch, processing, errors, transform } =
        useForm({
            ...
            cost_type_id: invoice?.cost_type_id ? String(invoice.cost_type_id) : '',
            project_id: invoice?.project_id ? String(invoice.project_id) : 'none',
        ...
        <form
            ...
            onSubmit={(event) => {
                ...
                transform((values) => ({
                    ...
                    cost_type_id: values.cost_type_id ? Number(values.cost_type_id) : null,
            ...
            <fieldset className="space-y-6" disabled={disabled || processing}>
                <div className="grid grid-cols-2 gap-4">
                    <div className="grid gap-2">
                        ...
                        <Select value={data.supplier_id} onValueChange={selectSupplier}>
                            <SelectTrigger id="supplier_id"><SelectValue placeholder="Lieferant wählen" /></SelectTrigger>
                            <SelectContent>
                        ...
                        <InputError message={errors.supplier_id ?? errors.lock_version} />
                    </div>
                    <div className="grid gap-2">
                        <Label htmlFor="supplier_invoice_no">Rechnungsnummer des Lieferanten</Label>
                        <Input id="supplier_invoice_no" value={data.supplier_invoice_no} onChange={(e) => setData('supplier_invoice_no', e.target.value)} />
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="grid gap-2">
                        ...
                        <Label className="flex items-center gap-2 text-sm font-normal">
                            <Checkbox checked={data.date_estimated} onCheckedChange={(checked) => setData('date_estimated', checked === true)} />
                ...
                <div className="grid grid-cols-3 gap-4">
                    ...
                    <div className="grid gap-2">
                        ...
                        <Input id="vat_rate" type="number" step="0.01" min="0" max="100" value={data.vat_rate} onChange={(e) => setData('vat_rate', e.target.value)} disabled={data.reverse_charge} required />
                        <InputError message={errors.vat_rate} />
                    </div>
                </div>

                <Label className="flex items-center gap-2 text-sm font-normal">
                    <Checkbox checked={data.reverse_charge} onCheckedChange={(checked) => setData('reverse_charge', checked === true)} />
                    §19 / Reverse Charge (Übergang der Steuerschuld — Satz 0, USt 0)
                </Label>

                <div className="grid grid-cols-2 gap-4">
                    <div className="grid gap-2">
                        <Label htmlFor="cost_type_id">Kostenart</Label>
                        <Select value={data.cost_type_id} onValueChange={(value) => setData('cost_type_id', value)}>
                            <SelectTrigger id="cost_type_id"><SelectValue placeholder="Kostenart wählen" /></SelectTrigger>
                            <SelectContent>
                                {costTypes.map((costType) => (
                                    <SelectItem key={costType.id} value={String(costType.id)}>{costType.name}</SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="grid gap-2">
                        <Label htmlFor="payment_due_on">Zahlbar bis (leer = Lieferanten-Ziel)</Label>
                        <Input id="payment_due_on" type="date" value={data.payment_due_on} onChange={(e) => setData('payment_due_on', e.target.value)} />
                        <InputError message={errors.payment_due_on} />
                    </div>
                    <div className="grid gap-2">
                        <Label htmlFor="payment_method">Zahlungsart</Label>
                        <Input id="payment_method" value={data.payment_method} onChange={(e) => setData('payment_method', e.target.value)} placeholder="z. B. Überweisung, Abbucher" />
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="grid gap-2">
                        ...
                        <Input id="skonto_amount" type="number" step="0.01" min="0" value={data.skonto_amount} onChange={(e) => setData('skonto_amount', e.target.value)} />
                        <InputError message={errors.skonto_amount} />
                    </div>
                    <div className="grid gap-2">
                        <Label htmlFor="skonto_until">Skonto bis</Label>
                        <Input id="skonto_until" type="date" value={data.skonto_until} onChange={(e) => setData('skonto_until', e.target.value)} />
